Hey, I'm Şiyar 👋

AI Engineering student at Hacettepe University · Ankara, Turkey  
I like understanding how things work — then building them.

---

### 🛠️ Tech Stack

**Languages**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)

**Robotics & Systems**  
![ROS2](https://img.shields.io/badge/ROS_2-22314E?style=flat&logo=ros&logoColor=white)
![Gazebo](https://img.shields.io/badge/Gazebo-F58113?style=flat&logoColor=white)
![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=flat&logo=ubuntu&logoColor=white)

---

### 🚀 Projects

| Project | Description | Stack |
|---|---|---|
| [sanal-lab](https://github.com/Siyarai/sanal-lab) | Interactive virtual lab for physics & chemistry | JavaScript |
| [Barkod-Okuyucusu](https://github.com/Siyarai/Barkod-Okuyucusu) | POS & inventory management with barcode reader | Python, CustomTkinter |
| HepsiBurada Address Hackathon | Address resolution model — HepsiBurada hackathon | Python, CatBoost |

---

### 📌 Currently focused on

- OOP with Java & C++ (data structures, algorithms)
- ROS 2 on Ubuntu — working toward hardware/software integration
- Integrating AI models into robotic systems

**Siyarai/Siyarai** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
